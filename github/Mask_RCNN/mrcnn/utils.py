...
class Dataset(object):
    ...
    def load_image(self, image_id):
        ...
        pnm_read(self.image_info[image_id]['path'])
        # print('версия вани')
        # imrows = 2048  # 2048
        # imcols = 2448  # 2448
        # imsize = imrows * imcols
        # starttime = time.time()
        # a1 = np.array([i * 4 for i in range(512)])
        # b1 = np.array([1 + i * 4 for i in range(512)])
        # c1 = np.concatenate((a1, b1))
        # c1 = np.sort(c1)
        # a2 = np.array([i * 4 for i in range(612)])
        # b2 = np.array([1 + i * 4 for i in range(612)])
        # c2 = np.concatenate((a2, b2))
        # c2 = np.sort(c2)
        # with open(self.image_info[image_id]['path']) as rawimage:
        #     img = np.fromfile(rawimage, np.dtype(np.uint8), imsize).reshape((imrows, imcols))
        #     print(img.shape)
        # img = img[c1, :]
        # img = img[:, c2]
        # colour = cv2.demosaicing(img, cv2.COLOR_BAYER_GR2RGBA)
        # image = cv2.cvtColor(colour, cv2.COLOR_RGBA2RGB)
